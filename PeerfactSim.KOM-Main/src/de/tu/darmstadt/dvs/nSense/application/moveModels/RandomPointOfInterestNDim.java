 ...
 * @version 01/11/2013
 */
public class RandomPointOfInterestNDim implements IMoveModelNDim {

	/**
	 * Logger for this class
	 */
	final static Logger log = SimLogger
			.getLogger(RandomPointOfInterestNDim.class);
	 ...
	 * this class.
	 */
	public RandomPointOfInterestNDim() {
		...
		this.RandomPOI = new int[1][4];
		this.RandomPOI[0][0] = 560;
		this.RandomPOI[0][1] = 450;
		this.RandomPOI[0][2] = 600;
		this.RandomPOI[0][3] = 550;
	}
	 ...
	 */
	public RandomPointOfInterestNDim(int moveSpeedLimit,
			VectorN worldDimensions, double speedChaningRate) {
	...
	private VectorN nextTarget() {
		float[] dimensions = new float[worldDimensions.getSize()];
		int poi = new Random().nextInt(this.RandomPOI.length);
		int[] rpoi = this.RandomPOI[poi];
		for (int i = 0; i < worldDimensions.getSize(); i++) {
			dimensions[i] = rpoi[i];
		}
